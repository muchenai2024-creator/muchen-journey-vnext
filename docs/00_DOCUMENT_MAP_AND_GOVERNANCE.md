...
| 23 | [G4–G6 下一批工作包定义](23_G4_G6_NEXT_WORK_PACKAGES.md) | WP-07～WP-15 如何按单一 WIP 推进候选、试点与正式切换？ | WP-07 已关闭；WP-08 已切换为冻结基础设施的 Alpha 最小部署路径，尚未部署成功；WP-09～WP-15 未激活 |
...
| 26 | [WP-08 火山引擎 Staging 实施路径证据](26_WP08_VOLCENGINE_STAGING_PATH_EVIDENCE.md) | 已锁定 provider/region/budget 后，唯一 IaC/CI/secret/回滚路径是什么，云端是否已经写入？ | provision 已冻结；Alpha 已进入 migration 容器但 RDS 超时，后续只读审计确认现有 AllowList 安全组绑定的有效 `IpList` 缺失；等待一次精确同步授权，migration/TLS/UAT 尚未运行 |
