...
| 26 | [WP-08 火山引擎 Staging 实施路径证据](26_WP08_VOLCENGINE_STAGING_PATH_EVIDENCE.md) | 已锁定 provider/region/budget 后，唯一 IaC/CI/secret/回滚路径是什么，云端是否已经写入？ | provision 已冻结；Alpha 已进入 migration 容器但 RDS 超时；控制台同步后绑定与派生 IP 已匹配，但实例 `IsLatest=false`，等待有界只读复验，migration/TLS/UAT 尚未运行 |
