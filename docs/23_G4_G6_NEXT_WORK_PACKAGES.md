...
截至 2026-07-24，独立 staging 基础资源、RDS CA 和 remote state 已收敛。Alpha run `30062128087` 使用主线 `a4bf1775e0de25d10dc17b678421e2dae500101c`，按设计跳过 DNS/import/plan/apply，通过冻结 state、精确 `/32`、私有 bundle、release-local secret 与 Compose 校验；随后 ECS 访问 Docker Hub 拉取固定 Caddy digest 时超时。SSH 已关闭且没有重试。受控 mirror run `30063385826` 已把该固定源 digest 复制到项目 GHCR 并验证目标 digest，Compose 现固定到该 GHCR digest。当前状态为 `ALPHA_PILOT_EDGE_MIRROR_READY`；新的 deploy、真实身份、真人 UAT 与 WP-09 均不得提前启动或转绿。
