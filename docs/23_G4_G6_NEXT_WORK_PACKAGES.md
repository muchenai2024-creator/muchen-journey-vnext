...
截至 2026-07-24，独立 staging 基础资源、RDS CA 和 remote state 已收敛。Alpha run `30026998583` 已证明 deploy 会跳过 DNS/import/plan/apply，并能读取冻结 state、打开和撤销精确 `/32`；但发布脚本在 migration 前错误读取全局 secret 目录而停止，未重试。单行 release-local secret 路径修复与测试已就绪，当前状态为 `ALPHA_PILOT_SECRET_PATH_FIX_READY`；新的 deploy 仍需独立授权，真实身份、真人 UAT 与 WP-09 不得提前启动或转绿。
