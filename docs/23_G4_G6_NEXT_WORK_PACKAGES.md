...
截至 2026-07-23，独立 staging 基础资源、RDS CA 和 remote state 已收敛。唯一授权的 deploy run `30020569136` 越过基础设施和临时 SSH 后，在 migration/容器启动前因既有 ECS 缺少创建期候选标记停止，SSH 入口已撤销且未重试。按硬停止条件，WP-08 已将同一 workflow 的 Alpha `deploy` 收窄为冻结 state 仅读取、临时 `/32`、发布包、Compose 和 TLS smoke；DNS/import/plan/apply 只保留在显式 `provision`。当前状态为 `ALPHA_PILOT_PATH_READY`：新的 Alpha deploy 尚未执行，真实身份、真人 UAT 与 WP-09 仍不得提前启动或转绿。
