...
2026-07-22 用户进一步锁定火山引擎、华北2（北京）`cn-beijing`、按量计费、独立 IAM/VPC/SG/ECS/RDS/TOS/staging 域名。首次 ¥500/月尝试因成本超限停止，云端未写入。用户随后将预算提高为 ¥800/月并保留托管 RDS，完成 Next.js 16.2.11 / sharp 0.35.3 安全修复；新主线候选 `670661865f708a835997596ed5b74904809564a5` 的 CI、候选打包和 GHCR digest 均通过，并获当前对话精确创建与部署授权。同日刷新后的月预测为 ¥656.26，距上限余 ¥143.74。当前状态为 `AUTHORIZED_CANDIDATE_BOUND_APPLY_PENDING`：候选绑定变更须先进入受保护主线，再执行唯一 staging workflow；物理验收完成前 WP-09 仍不得启动。
