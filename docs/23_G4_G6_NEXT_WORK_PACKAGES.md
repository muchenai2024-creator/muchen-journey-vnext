...
截至 2026-07-23，独立项目中的部分 IAM、网络、ECS、RDS/TOS 和 DNS 基础资源已由受审 workflow 创建或纳管，但应用尚未部署。第二次 provision run `29945430858` 的 saved plan 为 `2 add / 5 change / 0 destroy`，破坏性门禁通过；apply 在 RDS AllowList 空 `IpList` 序列化和缺少全局只读 `dns:QueryRecord` 两处停止。代码修复已通过 PR #17 合并到主线 `1791ea6d89a290cf4ff41e5c4a9e27fb64d7213c`，远端 required check 通过。所需 DNS 权限只允许是全局 `dns:QueryRecord` 单项；策略当前仅已准备，尚未获得明确提交授权，也未附加。当前状态为 `PROVISION_REMEDIATION_WAITING_IAM_CONFIRMATION`：未执行 migration、seed、应用容器、TLS 或 browser smoke，候选 deployment 为 `NOT_RUN`，WP-09 仍不得启动。
