...
| WP-08 物理独立 Staging | ISO-MUST-003/004/007/008/009/012；DEC-003/013/014；AT-ISO-002/003/005；AT-ARCH-002/003/005 | `make wp08-staging-readiness`、`make wp08-staging-apply-check`、`make wp08-workflow-check`、Terraform validate、saved-plan destroy/replacement guard，以及脱敏只读 `phase=audit`；DoR 与路径证据见 25/26 | provision 已冻结；mirror run 30063385826 与 Compose GHCR digest 已关闭 Docker Hub 阻塞。唯一 deploy run 30063847635 拉取四镜像成功，但第一次 Alembic 连接 RDS 超时；控制台同步后 run 30067829879 已证明正确绑定和 IP 匹配，仅实例 `IsLatest=false`。migration、应用、TLS、身份与 UAT 仍 `NOT_RUN`，SSH 已关闭且未重试 | `ALPHA_PILOT_RDS_INSTANCE_SYNC_UNVERIFIED`；保留 `IsLatest=true` 硬门禁并修正单次 audit 的异步传播等待，WP-09 未激活，整体发布仍 `NO_GO` |
