...
| WP-08 物理独立 Staging | ISO-MUST-003/004/007/008/009/012；DEC-003/013/014；AT-ISO-002/003/005；AT-ARCH-002/003/005 | `make wp08-staging-readiness`、`make wp08-staging-apply-check`、`make wp08-workflow-check`、Terraform validate 与 saved-plan destroy/replacement guard；DoR 与路径证据见 25/26 | run 29994013611 已完成 provision 收敛；新 RDS CA 已写入 GitHub `staging` Environment。唯一 deploy run 29997923817 的 `0 add / 5 change / 0 destroy` plan 与破坏性门禁通过，但 CloudControl 更新安全组时再次把空 PrefixList 引用纳入鉴权而停止；`always()` 清理已恢复 SSH 关闭态。migration、应用部署、TLS 和物理隔离验收均 `NOT_RUN` | `DEPLOY_STOPPED_SECURITY_GROUP_MODEL_REMEDIATION_REQUIRED`；WP-09 未激活，整体发布仍 `NO_GO` |
